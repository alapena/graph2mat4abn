...
def main():
    ...
    savedir = Path('results_dos/h_crystalls_15/full_range')
    split = "train"
    only_true = False
    ...
    for i, path in enumerate(paths):
        ...
        with torch.no_grad():
            ...
            h_pred_std = processor.matrix_from_data(data, predictions={"node_labels": model_predictions["node_labels"], "edge_labels": model_predictions["edge_labels"]})[0]
            x_std = processor.matrix_from_data(data)[0]
            ...
            x_std.data = (x_std.data * std) + mean
            h_true = x_std #Recovered

            h_pred_std.data = (h_pred_std.data * std) + mean
            h_pred = h_pred_std #Recovered
        ...
        filepath = savedir / f"{split}_{n_atoms}atm_{structure}.png"
        fig.write_image(str(filepath))
        filepath = savedir / f"{split}_{n_atoms}atm_{structure}.html"
